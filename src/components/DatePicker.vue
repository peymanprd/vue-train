            ...
            <table>
                <div v-if="state.showMonthList" class="datePicker__div">
                    <button
                        ...
                        :key="index"
                        :class="{
                            'datePicker--active': state.month === ++index,
                        }"
                ...
                <tbody>
                    <tr
                        v-for="(item, index) in state.calcNumberWeeks(
                            state.offsetDay(
                                state.firstDayMonth(state.year, state.month)
                            ),
                            state.lastOffsetDay(
                                state.lastDayMonth(
                                    state.year,
                    ...
                    >
                        <td
                            ...
                            :key="idx"
                            :class="{
                                'datePicker__td--disable':
                                    typeof day !== 'number',
                                'datePicker__td--active':
                                    state.selected === day,
                                'datePicker--active':
                                    !state.selected &&
                                    state.today === day &&
                                    state.checkYearMonth(
                                        state.year,
                                        state.month
                                    ),
...
export default {
    ...
    setup(props, context) {
        ...
        const state = reactive({
            ...
            calcNumberWeeks: computed(
                () => (offsetDay, lastOffsetDay, daysInMonth) => {
                    return Math.floor(
                        (offsetDay + lastOffsetDay + daysInMonth) / 7
                    )
                }
            ),
            tableDays: computed(
        ...
        const dateSelectedHandle = (event = null, index) => {
            if (
                event &&
                event.target.classList['value'].indexOf(
                    'datePicker__td--disable'
                ) > -1
            )
                return
        ...
        const genMatrix = (n, length, firstItems, lastItems) => {
            ...
            genArrNum = [
                ...genArrNum,
                ...lastItems.map(item => item.toString()),
            ]
